...
export async function onRequest({ request, env }) {
  try {
      ...
      return jsonResponse(
        {
          success: false,
          message:
            'Server login belum dikonfigurasi: GITHUB_PAT tidak tersedia di environment Cloudflare Pages.'
        },
        500
      );
    }
      ...
      return jsonResponse(
        {
          success: false,
          message:
            'Gagal mengambil data user dari GitHub. Cek: token, permission, atau lokasi user.json.',
          detail: txt
        },
        500
      );
      ...
      return jsonResponse(
        {
          success: false,
          message:
            'File user.json bukan JSON valid. Pastikan format JSON benar tanpa koma gantung.',
    ...
    return jsonResponse(
      {
        success: false,
        message: 'Terjadi kesalahan internal di server login.',
...
function generateToken(username) {
  const base = `${username}:${Date.now()}:${Math.random()
    .toString(36)
    .slice(2, 10)}`;
  return btoa(base);
}
